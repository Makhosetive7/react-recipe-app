...
const CocktailsDetailsModal = ({ recipeId, onClose }) => {
  ...
  const fetchCocktailDetails = useCallback(async () => {
    ...
    try {
      ...
      if (data.drinks && data.drinks[0]) {
        ...
        setError("Cocktail recipe not found");
      }
    } catch (err) {
        ...
        {loading ? (
          <LoadingState>
            <Circles
              ...
              visible={true}
            />
            <LoadingText>Mixing up the details...</LoadingText>
          </LoadingState>
        ) : error ? (
          <ErrorState>
            <ErrorIcon>🍸</ErrorIcon>
            <ErrorTitle>Unable to Load Cocktail</ErrorTitle>
            <ErrorMessage>{error}</ErrorMessage>
            <RetryButton onClick={fetchCocktailDetails}>
              Try Again
            </RetryButton>
          </ErrorState>
        ) : item ? (
          <ModalContent>
              ...
              <CocktailMeta>
                ...
                <MetaGrid>
                  ...
                  <MetaItem>
                    ...
                    <MetaContent>
                      <MetaLabel>Glass Type</MetaLabel>
                      <MetaValue>{item.strGlass}</MetaValue>
                    </MetaContent>
                  </MetaItem>
                  <MetaItem>
                    <MetaIcon>
                    ...
                    <MetaContent>
                      ...
                      <MetaValue>{item.strAlcoholic || "Alcoholic"}</MetaValue>
                    </MetaContent>
                  </MetaItem>
                </MetaGrid>
              </CocktailMeta>
            ...
            <TabContent>
              {activeTab === "instructions" && (
                <InstructionsSection>
                  <SectionTitle>Mixing Instructions</SectionTitle>
                  <InstructionsText>
                    {item.strInstructions ? (
                      ...
                      <InstructionParagraph>
                        No specific instructions provided. Mix all ingredients together and serve in a {item.strGlass}.
                      </InstructionParagraph>
                    )}
                  </InstructionsText>
                </InstructionsSection>
              ...
              {activeTab === "ingredients" && (
                <IngredientsSection>
                  <SectionTitle>Cocktail Ingredients</SectionTitle>
                  <IngredientsGrid>
                    {ingredients.length > 0 ? (
                      ...
                      <EmptyIngredients>
                        No specific ingredients listed for this cocktail.
                      </EmptyIngredients>
                    )}
                  </IngredientsGrid>
                </IngredientsSection>
...
const EmptyIngredients = styled.div`
  text-align: center;
  padding: 2rem;
  font-family: 'Inter', sans-serif;
  color: #666;
...
export default CocktailsDetailsModal;